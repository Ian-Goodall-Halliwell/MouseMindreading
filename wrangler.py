...
import os
from download import dl_alldata
from download_lfp import dl_st
def preprocess(minamnt=10,reg=None,thresh=None):
    if os.path.exists("alldata.npy"):
        alldat = np.load("alldata.npy", allow_pickle=True)
    else:
        dl_alldata()
        alldat = np.load("alldata.npy", allow_pickle=True)
    if os.path.exists("dat_st.npy"):
